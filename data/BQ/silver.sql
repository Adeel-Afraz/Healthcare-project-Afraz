...
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.departments` (
    Dept_Id STRING,
    SRC_Dept_Id STRING,
    Name STRING,
    datasource STRING,
...
TRUNCATE TABLE `avd-practice-1.Project_Silver.departments`;

-- 3. full load by Inserting merged Data 
INSERT INTO `avd-practice-1.Project_Silver.departments`
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `avd-practice-1.Project_Bronze.departments_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `avd-practice-1.Project_Bronze.departments_hb`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create table providers by Merge Data from Hospital A & B  
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.providers` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
-- 2. Truncate Silver Table Before Inserting 
TRUNCATE TABLE `avd-practice-1.Project_Silver.providers`;

-- 3. full load by Inserting merged Data 
INSERT INTO `avd-practice-1.Project_Silver.providers`
SELECT DISTINCT 
    ProviderID,
    FirstName,
    LastName,
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `avd-practice-1.Project_Bronze.providers_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `avd-practice-1.Project_Bronze.providers_hb`
);

-------------------------------------------------------------------------------------------------------

-- 1. Create patients Table in BigQuery
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.patients` (
    Patient_Key STRING,
...
CREATE OR REPLACE TABLE `avd-practice-1.Project_Silver.quality_checks` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `avd-practice-1.Project_Bronze.patients_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `avd-practice-1.Project_Bronze.patients_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `avd-practice-1.Project_Silver.patients` AS target
USING `avd-practice-1.Project_Silver.quality_checks` AS source
...
DROP TABLE IF EXISTS `avd-practice-1.Project_Silver.quality_checks`;

-------------------------------------------------------------------------------------------------------

-- 1. Create transactions Table in BigQuery
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.transactions` (
...
SELECT DISTINCT 
    ...
    END AS is_quarantined
FROM (
    SELECT DISTINCT *, 'hosa' AS datasource FROM `avd-practice-1.Project_Bronze.transactions_ha`
    UNION ALL
    SELECT DISTINCT *, 'hosb' AS datasource FROM `avd-practice-1.Project_Bronze.transactions_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `avd-practice-1.Project_Silver.transactions` AS target
USING `avd-practice-1.Project_Silver.quality_checks` AS source
...
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.encounters` (
...
CREATE OR REPLACE TABLE `avd-practice-1.Project_Silver.quality_checks_encounters` AS
SELECT DISTINCT 
...
FROM (
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `avd-practice-1.Project_Bronze.encounters_ha`
    ...
    SELECT DISTINCT 
        ...
        ModifiedDate,
        'hosb' AS datasource
    FROM `avd-practice-1.Project_Bronze.encounters_hb`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `avd-practice-1.Project_Silver.encounters` AS target
USING `avd-practice-1.Project_Silver.quality_checks_encounters` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `avd-practice-1.Project_Silver.quality_checks_encounters`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the Claims Table in BigQuery
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.claims` (
    Claim_Key STRING,
    SRC_ClaimID STRING,
...
CREATE OR REPLACE TABLE `avd-practice-1.Project_Silver.quality_checks_claims` AS
SELECT 
...
FROM (
    SELECT 
        ...
        ModifiedDate,
        'hosa' AS datasource
    FROM `avd-practice-1.Project_Bronze.claims`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `avd-practice-1.Project_Silver.claims` AS target
USING `avd-practice-1.Project_Silver.quality_checks_claims` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `avd-practice-1.Project_Silver.quality_checks_claims`;

-------------------------------------------------------------------------------------------------------

-- 1. Create the CP Codes Silver Table in BigQuery
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Silver.cpt_codes` (
    CP_Code_Key STRING,
...
CREATE OR REPLACE TABLE `avd-practice-1.Project_Silver.quality_checks_cpt_codes` AS
SELECT 
...
FROM (
    SELECT 
        ...
        code_status,
        'hosa' AS datasource
    FROM `avd-practice-1.Project_Bronze.cpt_codes`
);

-- 3. Apply SCD Type 2 Logic with MERGE
MERGE INTO `avd-practice-1.Project_Silver.cpt_codes` AS target
USING `avd-practice-1.Project_Silver.quality_checks_cpt_codes` AS source
...
-- 4. DROP quality_check table
DROP TABLE IF EXISTS `avd-practice-1.Project_Silver.quality_checks_cpt_codes`;
