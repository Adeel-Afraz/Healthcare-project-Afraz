...
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Gold.provider_charge_summary` (
...
# truncate table
TRUNCATE TABLE `avd-practice-1.Project_Gold.provider_charge_summary`;

# insert data
INSERT INTO `avd-practice-1.Project_Gold.provider_charge_summary`
SELECT 
    CONCAT(p.firstname, ' ', p.LastName) AS Provider_Name,
    d.Name AS Dept_Name,
    SUM(t.Amount) AS Amount
FROM `avd-practice-1.Project_Silver.transactions` t
LEFT JOIN `avd-practice-1.Project_Silver.providers` p 
    ON SPLIT(p.ProviderID, "-")[SAFE_OFFSET(1)] = t.ProviderID
LEFT JOIN `avd-practice-1.Project_Silver.departments` d 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Gold.patient_history` (
    Patient_Key STRING,
    FirstName STRING,
    LastName STRING,
    Gender STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `avd-practice-1.Project_Gold.patient_history`;

# INSERT DATA
INSERT INTO `avd-practice-1.Project_Gold.patient_history`
SELECT 
    p.Patient_Key,
    p.FirstName,
    p.LastName,
    ...
    c.PayorType
FROM `avd-practice-1.Project_Silver.patients` p
LEFT JOIN `avd-practice-1.Project_Silver.encounters` e 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = e.PatientID
LEFT JOIN `avd-practice-1.Project_Silver.transactions` t 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = t.PatientID
LEFT JOIN `avd-practice-1.Project_Silver.claims` c 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Gold.provider_performance` (
    ProviderID STRING,
    FirstName STRING,
    LastName STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `avd-practice-1.Project_Gold.provider_performance`;

# INSERT DATA
INSERT INTO `avd-practice-1.Project_Gold.provider_performance`
SELECT 
    pr.ProviderID,
    pr.FirstName,
    pr.LastName,
...
FROM `avd-practice-1.Project_Silver.providers` pr
LEFT JOIN `avd-practice-1.Project_Silver.encounters` e 
...
# CREATE TABLE
CREATE TABLE IF NOT EXISTS `avd-practice-1.Project_Gold.department_performance` (
    Dept_Id STRING,
...
# TRUNCATE TABLE
TRUNCATE TABLE `avd-practice-1.Project_Gold.department_performance`;

# INSERT DATA
INSERT INTO `avd-practice-1.Project_Gold.department_performance`
SELECT 
    d.Dept_Id,
...
FROM `avd-practice-1.Project_Silver.departments` d
LEFT JOIN `avd-practice-1.Project_Silver.encounters` e 
