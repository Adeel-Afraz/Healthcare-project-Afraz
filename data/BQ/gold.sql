...
FROM `avd-practice-1.Project_Sliver.transactions` t
LEFT JOIN `avd-practice-1.Project_Sliver.providers` p 
    ON SPLIT(p.ProviderID, "-")[SAFE_OFFSET(1)] = t.ProviderID
LEFT JOIN `avd-practice-1.Project_Sliver.departments` d 
...
SELECT 
    ...
    c.PayorType
FROM `avd-practice-1.Project_Sliver.patients` p
LEFT JOIN `avd-practice-1.Project_Sliver.encounters` e 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = e.PatientID
LEFT JOIN `avd-practice-1.Project_Sliver.transactions` t 
    ON SPLIT(p.Patient_Key, '-')[OFFSET(0)] || '-' || SPLIT(p.Patient_Key, '-')[OFFSET(1)] = t.PatientID
LEFT JOIN `avd-practice-1.Project_Sliver.claims` c 
...
FROM `avd-practice-1.Project_Sliver.providers` pr
LEFT JOIN `avd-practice-1.Project_Sliver.encounters` e 
...
FROM `avd-practice-1.Project_Sliver.departments` d
LEFT JOIN `avd-practice-1.Project_Sliver.encounters` e 
